...
// ──────────────────────── /api/check-urls ────────────────────────
// A model asked to "find a job posting" will sometimes hallucinate a URL,
// or hand back a generic search-results/careers-homepage link instead of
// the specific listing. The LLM-based verify step only judges plausibility
// from search snippets — it never actually confirms the link resolves. This
// endpoint does a real HTTP check server-side (the browser can't fetch
// arbitrary third-party origins itself) so obviously dead links (404/410/5xx)
// can be dropped before they ever reach the user.
